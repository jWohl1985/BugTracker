...
namespace BugTracker.Data
{
    ...
    public static class DataUtility
    {
        ...
        public static async Task SeedDefaultProjectPriorityAsync(ApplicationDbContext context)
        {
            try
            {
                IList<ProjectPriority> projectPriorities = new List<ProjectPriority>() {
                                                    new ProjectPriority() { Name = ProjPriority.Low.ToString() },
                                                    new ProjectPriority() { Name = ProjPriority.Medium.ToString() },
                                                    new ProjectPriority() { Name = ProjPriority.High.ToString() },
                                                    new ProjectPriority() { Name = ProjPriority.Urgent.ToString() },
                };
        ...
        public static async Task SeedDefautProjectsAsync(ApplicationDbContext context)
        {
            ...
            int priorityLow = context.ProjectPriorities.FirstOrDefault(p => p.Name == ProjPriority.Low.ToString())!.Id;
            int priorityMedium = context.ProjectPriorities.FirstOrDefault(p => p.Name == ProjPriority.Medium.ToString())!.Id;
            int priorityHigh = context.ProjectPriorities.FirstOrDefault(p => p.Name == ProjPriority.High.ToString())!.Id;
            int priorityUrgent = context.ProjectPriorities.FirstOrDefault(p => p.Name == ProjPriority.Urgent.ToString())!.Id;
        ...
        public static async Task SeedDefaultTicketTypeAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketType> ticketTypes = new List<TicketType>() {
                     new TicketType() { Name = TickType.NewDevelopment.ToString() },      // Ticket involves development of a new, uncoded solution 
                     new TicketType() { Name = TickType.WorkTask.ToString() },            // Ticket involves development of the specific ticket description 
                     new TicketType() { Name = TickType.Defect.ToString()},               // Ticket involves unexpected development/maintenance on a previously designed feature/functionality
                     new TicketType() { Name = TickType.ChangeRequest.ToString() },       // Ticket involves modification development of a previously designed feature/functionality
                     new TicketType() { Name = TickType.Enhancement.ToString() },         // Ticket involves additional development on a previously designed feature or new functionality
                     new TicketType() { Name = TickType.GeneralTask.ToString() }          // Ticket involves no software development but may involve tasks such as configuations, or hardware setup
                };
        ...
        public static async Task SeedDefaultTicketStatusAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketStatus> ticketStatuses = new List<TicketStatus>() {
                    new TicketStatus() { Name = TickStatus.New.ToString() },                 // Newly Created ticket having never been assigned
                    new TicketStatus() { Name = TickStatus.Development.ToString() },         // Ticket is assigned and currently being worked 
                    new TicketStatus() { Name = TickStatus.Testing.ToString()  },            // Ticket is assigned and is currently being tested
                    new TicketStatus() { Name = TickStatus.Resolved.ToString()  },           // Ticket remains assigned to the developer but work is now complete
                };
        ...
        public static async Task SeedDefaultTicketPriorityAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketPriority> ticketPriorities = new List<TicketPriority>() {
                                                    new TicketPriority() { Name = TickPriority.Low.ToString()  },
                                                    new TicketPriority() { Name = TickPriority.Medium.ToString() },
                                                    new TicketPriority() { Name = TickPriority.High.ToString()},
                                                    new TicketPriority() { Name = TickPriority.Urgent.ToString()},
                };
        ...
        public static async Task SeedDefautTicketsAsync(ApplicationDbContext context)
        {
            ...
            //Get ticket type Ids
            int typeNewDev = context.TicketTypes.FirstOrDefault(p => p.Name == TickType.NewDevelopment.ToString())!.Id;
            int typeWorkTask = context.TicketTypes.FirstOrDefault(p => p.Name == TickType.WorkTask.ToString())!.Id;
            int typeDefect = context.TicketTypes.FirstOrDefault(p => p.Name == TickType.Defect.ToString())!.Id;
            int typeEnhancement = context.TicketTypes.FirstOrDefault(p => p.Name == TickType.Enhancement.ToString())!.Id;
            int typeChangeRequest = context.TicketTypes.FirstOrDefault(p => p.Name == TickType.ChangeRequest.ToString())!.Id;

            //Get ticket priority Ids
            int priorityLow = context.TicketPriorities.FirstOrDefault(p => p.Name == TickPriority.Low.ToString())!.Id;
            int priorityMedium = context.TicketPriorities.FirstOrDefault(p => p.Name == TickPriority.Medium.ToString())!.Id;
            int priorityHigh = context.TicketPriorities.FirstOrDefault(p => p.Name == TickPriority.High.ToString())!.Id;
            int priorityUrgent = context.TicketPriorities.FirstOrDefault(p => p.Name == TickPriority.Urgent.ToString())!.Id;

            //Get ticket status Ids
            int statusNew = context.TicketStatuses.FirstOrDefault(p => p.Name == TickStatus.New.ToString())!.Id;
            int statusDev = context.TicketStatuses.FirstOrDefault(p => p.Name == TickStatus.Development.ToString())!.Id;
            int statusTest = context.TicketStatuses.FirstOrDefault(p => p.Name == TickStatus.Testing.ToString())!.Id;
            int statusResolved = context.TicketStatuses.FirstOrDefault(p => p.Name == TickStatus.Resolved.ToString())!.Id;
